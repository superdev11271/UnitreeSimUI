...
const SPEED_DIFF_WINDOW_SEC = 0.3;
const SPEED_MIN_WINDOW_SEC = 0.2;
const SPEED_HISTORY_MAX_SEC = 0.5;
const SPEED_SMOOTHING = 0.45;
const SPEED_STOP_THRESHOLD = 0.03;

const _poseQuat = new THREE.Quaternion();
const _velocityWorld = new THREE.Vector3();
const _velocityBody = new THREE.Vector3();
...
class WorldViewer {
  ...
  updateBodySpeed(message, pose) {
    const receiveSec = performance.now() * 0.001;
    ...
    if (rawSpeed !== null) {
      ...
      this.bodySpeedMps = this.hasBodySpeed
        ? this.bodySpeedMps * (1 - SPEED_SMOOTHING) + rawSpeed * SPEED_SMOOTHING
        : rawSpeed;
